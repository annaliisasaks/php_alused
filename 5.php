...
<?php
// ülesanne 5, Anna-Liisa Saks, 02.12.2020
$break = "<br>";
$gurls = array('mari', 'kati', 'jaana', 'miku', 'jane', 'liisa', 'paula', 'grete');
...
$sum = 0;
for ($z = 0; $z<=count($palgad)-1; $z ++) {
    $sum += $palgad[$z];
}

echo $break."Palk: ".$break;
...
echo '<pre>'; print_r($firmad); echo '</pre>';

?>

<form action="5.php" method="get">
    <label for="firma">Sisesta firma nimi, mida tahad kustutada: </label>
...
echo "Kustutatud firma nimega massiiv: ".$break;
if (isset($_GET['firma']) or isset($_GET['firma'])) {
    if(strlen($_GET['firma']) and strlen($_GET['firma'])) {
        for ($k = 0; $k<=count($firmad)-1; $k++) {
            if ($firma == $firmad[$k]) {
                continue;
            } else {
                echo $firmad[$k].$break;
...
$pikim=$riigid[0];
for($r=0; $r<=count($riigid)-1; $r ++) {
    if (strlen($riigid[$r]) > strlen($pikim)) {
        $pikim = $riigid[$r];
    }else {
        continue;
    }
...
sort($hiina);
echo "Esimene : ".$hiina[0].$break."Viimane: ".$hiina[count($hiina)-1];
echo $break.$break."Google: ".$break;
?>
<form action="5.php" method="get">
    <label for="go">Sisesta kasutajanimi: </label>
    <input type="text" name="go" id="go" ><br>
...
$knimi = $_GET['go'] ??'';
$olemas = "ei ole massiivis";
for ($r=0; $r<=count($google)-1; $r ++) {
    if($knimi == $google[$r]) {
        $olemas="on massiivis";
    } else {
        continue;
...
?>
<div class="alert alert-primary" role="alert" id="demo">
    <?php
    if (strlen($knimi)===0) {
        echo "Sisesta kasutajanimi";
    } else {
        echo "Sinu sisestatud kasutajanimi ".$olemas;
    }
    ?>
</div>
...
?>
<p>Kolmas pilt: </p><img src="<?php echo "img/".$pildid[2];?>">

<p>Kõik pildid: </p>
<img src="<?php echo 'img/'.$pildid[0];?>">
<img src="<?php echo 'img/'.$pildid[1];?>">

<img src="<?php echo 'img/'.$pildid[2];?>">

<img src="<?php echo 'img/'.$pildid[3];?>">

<img src="<?php echo 'img/'.$pildid[4];?>">

<img src="<?php echo 'img/'.$pildid[5];?>">
